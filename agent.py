...
def core_scheduling_decision(
    ...
    try:
        ...
        # 处理价格数据
        if electricity_prices and abs(len(electricity_prices) - (time_horizon_hours / time_step_hours)) < 0.1:
            try:
        ...
        if carbon_prices and abs(len(carbon_prices) - (time_horizon_hours / time_step_hours)) < 0.1:
            try:
...
if __name__ == "__main__":
    ...
    # 测试查询
    test_queries = [
        "什么是储能电站？",  # 知识咨询，不需要调用工具
        "额定容量为400MWh，额定功率为100MW，电价预测，对后面24小时的电价预测，时间间隔是30分钟，单位是元/kWh:[0.32, 0.24, 0.24, 0.25, 0.26, 0.3, 0.31, 0.29, 0.32, 0.37, 0.41, 0.44, 0.44, 0.45, 0.46, 0.47, 0.47, 0.47, 0.46, 0.42, 0.35, 0.29, 0.23, 0.24, 0.25, 0.29, 0.29, 0.23, 0.25, 0.22, 0.21, 0.21, 0.2, 0.18, 0.17, 0.16, 0.16, 0.13, 0.12, 0.11, 0.1, 0.1, 0.12, 0.12, 0.12, 0.14, 0.15, 0.17 ],"
        "请帮我做一个24小时间隔为0.5小时的决策调度",
        # 需要调用工具
        "我有一个500MWh的储能电站，当前SOC是60%，请制定24小时的充放电策略"  # 需要调用工具
    ]

    print("测试查询示例：")
    print(f"\n{2}. {test_queries[1]}")
    result = agent.process_query(test_queries[1])
    print(f"回答： {result}")
