...
2. **GPT-2+LoRA微调实现电池SOH预测，XGBoost完成电价与功率预测**

   第一步SOH的预测是考虑到管理员可能会提问到，这个电池还能使用多久这样的场景，我们通过GPT-2这种transformers架构的大模型对于SOH这种长时间的特征，能够很好的捕捉全局变量，通过LoRA微调实现预测，效果达到了95%，代码在gpt2.py中
   第二步XGBoost完成电价和功率预测，XGBoost也是我们数学建模竞赛中经常用到的一个数学建模的方法，他相对于上面Decode架构的模型来说，更适合短时场景，比如我们这里的电价和功率，最终要的不是长时间线，而是一些影响电价的特征，比如气温，节假日，等等

   电价预测结果图：
  <img width="1320" height="656" alt="image" src="https://github.com/user-attachments/assets/e92bedcf-6899-43c0-8024-5fa247158bcd" />

  功率预测结果图：
  <img width="1325" height="658" alt="image" src="https://github.com/user-attachments/assets/676e812a-5817-479f-a16f-262fadc09170" />

3. **全量微调BERT-base做意图识别**
   针对意图识别，分类的场景BERT的表现是经得起敲打的，我们也是用了它最经典的模型去做意图识别的模块，收集 每个场景/意图 300-500 条 样本数据
   定义意图类别：
      例如：
      schedule_charging （制定充电计划）
      query_energy_storage （查询储能状态）
      report_generation （生成报表）
      general_qa （一般性问答）
      fault_diagnosis （故障诊断）
      预测电池健康等等...这里只是做一个例子，真实的模型和训练代码就不做出来了
   
   它的模型参量十分小，约有1.2亿参数量，完全可以全量微调获得一个更好的效果，使用AdamW优化器，这是微调Transformer模型的标准且首选的优化器。它正确处理了权重衰减，能带来更稳定的训练和更好的泛化能力。损失函数上选择了交叉熵损失函数（nn.CrossEntropyLoss），因为意图识别是个单标签多分类任务。
   **核心训练策略**
      学习率调度：使用线性衰减与热身。先用一个很小的学习率（如 1e-7）训练几个“步”或“轮次”，让模型稳定进入训练状态，然后再线性增长到预设的最大学习率，之后在整个训练过程中线性衰减到0。

      早停：持续监控模型在验证集上的性能（如准确率或F1分数）。当性能在连续多个轮次内不再提升时，就停止训练，并回滚到验证集性能最好的那个模型检查点，以防止过拟合。

      梯度裁剪：设置一个梯度最大值（如 1.0），在反向传播时，如果梯度超过这个阈值，就将其裁剪到这个值。这可以防止梯度爆炸，稳定训练过程。
  
4. **根据上一步收集到的准确的资料来构建fassi向量库，并自己构建智能体流和一些约束条件的规则库，后续也可以自己添加**

  因为相对于其他大佬微调的AI来说，我们的数据集还是比较少的，因此呢我们需要用RAG的技术来增强我们模型的知识能力。
  我们寻找前沿论文，以及一些自己收集修改的一些数据，和网络高质量检索储能电站的相关定义知识，构建fassi向量库，开源只开源部分知识，有些数据比较宝贵，耗费人力、资源，这里是给大家学习一下，分享一下我的设计。

5. **工具设计**

  做这种储能电站调度问题，那么如果是让大模型完全的自我决策，自己寻找方法计算经济效益和物理场模型显然是不太现实和精准的，那么我们就需要自己构建一些工具来供大模型使用，比如合适的计算方法，多目标优化模型，来评估这样调度的经济效益，从而可以寻优，找到最好的调度方案。按LLM-Tool的调用机制，这边计算方法采用的是Gurobi数学规划求解器，是一个很优秀的商用求解器，需要自己去官网申请，免费的许可证就够了，如果做超过一天的调度可能需要专业的许可证，当然，我们自己也自主设计了粒子群算法，遗传算法等求解。

  另外也可以自己集成一些其他的工具，因为我希望这个模型在专业领域上不只是可以用作调度，同样可以用作其他的方面，比如知识问答，根据天气评估一些手段，措施，这样就需要集成一个天气API的工具。

  以上我们自主设计的工具是采用的旧版的Function Calling，现在在系统中加入了MCP服务的设计，还未发布。
